...
describe("Validation Rules", function(){
    describe('Equal Rule', function () {
        ...
        it('should be valid when strings equal', function (done) {
            ...
            const rule = new EqualValidationRule();
            rule.validate(modelResolver, "a", "hello").then(function(isValid: boolean){
                expect(isValid).to.be.true;
                done();
            }).catch(done);
        });
        ...
        it('should be valid when dates equal', function (done) {
            ...
            const rule = new EqualValidationRule();
            rule.validate(modelResolver, "a", new Date(1995, 11, 17)).then(function(isValid: boolean){
                expect(isValid).to.be.true;
                done();
            }).catch(done);
        });
        ...
        it('should be invalid when number does not equal comparison', function (done) {
            ...
            const rule = new EqualValidationRule();
            rule.validate(modelResolver, "a", 20).then(function(isValid: boolean){
                expect(isValid).to.be.false;
                done();
            }).catch(done);
        });
